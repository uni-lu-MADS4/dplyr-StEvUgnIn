...
glimpse(judgments) # Rows: 188 Columns: 158
```
...
```{r}
judgments |> select(gender, subject, age, starts_with("STAI"))
```
...
```{r}
judgments |> filter(STAI_pre > 65 & STAI_post > 40)
```
...
```{r}
judgments |> arrange(desc(STAI_pre))
```

#### For better readability, move the columns `subject`, `STAI_pre` and `STAI_post` to the first positions.

```{r}
judgments |> select(subject, STAI_pre, STAI_post, everything())
```
...
```{r}
stopifnot("Duplicates present in judgements." = nrow(judgments) - count(judgments) == 0)
```
...
judgments <- judgments |>
  mutate(
    STAI_pre_category = case_when(
      STAI_pre < 25 ~ "low",
      STAI_pre > 65 ~ "high",
      is.numeric(STAI_pre) ~ "normal",
      .default = NA
      ),
    .after = STAI_pre,
  ) |>
...
```{r}
judgments |> distinct(STAI_pre, STAI_pre_category)
```
...
```{r}
judgments |> reframe(across(starts_with("REI"), ~ .x / 5))
```
